...
public class Serveur {
	...
	static float averageDegre(Serveur s)
	{
		int i=0;
	...
	static HashMap<Integer, Integer> CalculWeight(ArrayList<Integer> compo) {
		...
		ArrayList<Integer> mis = Serveur.mis;
	...
	private static HashMap<Integer, Integer> calculRedPoint(
		...
		int c = 0;
		for (int i = 0; i < compo.size(); i++) {
			if (Serveur.mis.contains(compo.get(i)))
				cn.put(compo.get(i), compo.get(i));
		}
	...
	static HashMap<Integer, Integer> calculComp(ArrayList<Integer> compo) {
		...
		for (int i = 0; i < compo.size(); i++) {
			...
			for (int j = 0; j < HashM.get(compo.get(i)).size(); j++) {
				if (Serveur.mis.contains(HashM.get(compo.get(i)).get(j))) {
		...
		try {
			...
			for (int i = 0; i < HashM.size(); i++) {
				...
				for (int j = 0; j < a.size(); j++) {
					if(col.get(Serveur.nodes.get(s).getId()).equals("black")){
						if(col.get(Serveur.nodes.get(a.get(j)).getId()).equals("black")){
					float x = (float) Math
			...
			for (int i = 0; i < nodes.size(); i++) {
				float x = (float) Math.abs(Serveur.nodes.get(i).getPtX() + 12) / 20;
				float y = (float) Math
						.abs(Serveur.nodes.get(i).getPtY() + 12 - 500) / 20;
				if (Serveur.col.get(i).equals("black")) {
			...
			for (int i = 0; i < zoneInt.size(); i++) {
				float x = (float) Math
						.abs(Serveur.zoneInt.get(i).getPtX() + 12) / 20;
				float y = (float) Math
						.abs(Serveur.zoneInt.get(i).getPtY() + 12 - 500) / 20;
				fileStream.println("\\draw  (" + x + "," + y
						+ ")  node[sommetZ]{};");
			}
...
//			}
			for (int i = 0; i < nodes.size(); i++) {
				...
				float y = (float) Math
						.abs(Serveur.nodes.get(i).getPtY() + 12 - 500) / 20;
				if(Serveur.mis.contains(Serveur.nodes.get(i).getId()))
				{
					fileStream.println("\\draw  (" + x + "," + y
							+ ")  node[sommetB]{};");
				} else
		...
		try {
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				if(Serveur.col.get(i).equals("black"))
				{
					...
					if(Serveur.col.get(i).equals("gray"))
					fileStream.println("\\draw  (" + x + "," + y
							+ ")  node[sommet]{};");
					else
						fileStream.println("\\draw  (" + x + "," + y
	...
	static void miniServeur() {
		...
		for (int i = 0; i < zoneInt.size(); i++) // pour chaque zone interet
		{
			...
			for (int j = 0; j < Serveur.nodes.size(); j++)
				// add all node in the zone Interet
				if (Serveur.nodes.get(j).getInt()
						.contains(Serveur.zoneInt.get(i).getId())) {
					nodes.put(Serveur.nodes.get(j).getId(),
							Serveur.nodes.get(j));

				}
			zoneInt.add(Serveur.zoneInt.get(i));
																				...
																				// composant
			{
				int d = entryy.getKey();
				Node val = entryy.getValue();
				if (val.getInt().contains(Serveur.zoneInt.get(i).getId())) {
					ArrayList<Integer> fr = Serveur.HashM.get(val.getId());

					for (int s = 0; s < fr.size(); s++) {
						if (Serveur.col.get(fr.get(s)).equals("black")
								&& !Serveur.nodes
										.get(fr.get(s))
										.getInt()
										.contains(
												Serveur.zoneInt.get(i).getId())) {// connecter
																					// a
																					// une
																					// node
																					// black
							...
							if (!extB.contains(fr.get(s)))
								extB.add(fr.get(s));
							if (!nodes.containsKey(Serveur.nodes.get(fr.get(s))
									.getId()))

							{
								nodes.put(Serveur.nodes.get(fr.get(s)).getId(),
										Serveur.nodes.get(fr.get(s)));

							}
						} else if (Serveur.col.get(fr.get(s)).equals("gray")
								&& !Serveur.nodes
										.get(fr.get(s))
										.getInt()
										.contains(
												Serveur.zoneInt.get(i).getId())) {// connecter
																					...
																					// une
																					// gray
							ArrayList<Integer> fs = Serveur.HashM
									.get(fr.get(s));
							boolean okays = false;
							for (int po = 0; po < fs.size(); po++) {// si elle
																	...
																	// l'ajoute
																	// pas
								if (Serveur.col.get(fs.get(po)).equals("black")
										&& !Serveur.nodes
												.get(fs.get(po))
												.getInt()
												.contains(
														Serveur.zoneInt.get(i)
																.getId()))
									okays = true;
							}
							if (!okays) {
								if (!extG.contains(fr.get(s)))
									extG.add(fr.get(s));
								if (!nodes.containsKey(Serveur.nodes.get(
										fr.get(s)).getId()))

								{
									nodes.put(Serveur.nodes.get(fr.get(s))
											.getId(), Serveur.nodes.get(fr
											.get(s)));
																				...
																				// composant
			{
				int key = entryy.getKey();
				String val = entryy.getValue();
				Serveur.col.put(key, val);
				if (extB.contains(key))
					Serveur.col.put(key, "black");
			}
			phase4Mini(col, HashM, nodes, extB);
